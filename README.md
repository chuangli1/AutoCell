...
后端代码在'APP'文件中，你可以打开该文件后，运行 `pip install -r requirements.txt`安装相关依赖，之后执行 `python main.py` 运行后端服务器，当然之前你需要配置数据库，相关方法可以自行上网搜索，相关数据和连接，可以查看`/APP/db/readme.md`, 通过运行`/APP/db/tables.py完成数据库的初始化`.
